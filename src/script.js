 ...
 */
import { Notyf } from 'notyf'; // assumes bundler or module support; for <script> use global Notyf
...
function update(changedId='') {
  ...
  const baseYearly = profitKwh * price;

  // build data with year 0 start
  const data = [{ year: 0, yearly: 0, cumulative: 0 }];
  let cum = 0, cumPrev = 0;
  let isProfitable = false;
  let paybackExact = null;

  for (let y = 1; y <= life; y++) {
    const yearProfit = baseYearly * Math.pow(1 + inc, y - 1);
    cumPrev = cum;
    cum += yearProfit;
    data.push({ year: y, yearly: yearProfit, cumulative: cum });
    if (!isProfitable && cum >= installC) {
      // compute exact payback in years
      const frac = (installC - cumPrev) / yearProfit;
      paybackExact = (y - 1) + frac;
      isProfitable = true;
    }
  }
  ...
  // text summary
  const avgText = `Je spaart gemiddeld <strong>€${rnd(baseYearly)}</strong> per jaar, over ${life} jaar.`;
  ...
  drawChart(data, installC, paybackExact, isProfitable);
  drawTable(data, isProfitable);
}
